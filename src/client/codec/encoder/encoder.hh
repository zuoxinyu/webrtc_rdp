...
namespace remoting
{
using TimeDelta = webrtc::TimeDelta;
using TimeTicks = webrtc::Timestamp;
...
class WebrtcVideoEncoder
{
  public:
    struct FrameParams {
        ...
        // contain updated_region() from the current frame. Otherwise the active
        // map is not cleared before adding updated_region(), which means it
        // will contain a union of updated_region() from all frames since this
        // flag was last set. This flag is used to top-off video quality with
        // VP8.
    ...
    struct FrameStats {
        ...
        TimeTicks capture_started_time;
        TimeTicks capture_ended_time;
        TimeTicks encode_started_time;
        TimeTicks encode_ended_time;
        TimeDelta send_pending_delay{TimeDelta::Seconds(INT_MAX)};
        TimeDelta rtt_estimate{TimeDelta::Seconds(INT_MAX)};
    ...
    // request. SUCCEEDED with an empty EncodedFrame (nullptr) indicates the
    // frame should be dropped (unchanged or empty frame). Otherwise
    // EncodeResult shows the errors.
    using EncodeCallback = int;
    ...
    // portions of the frame to higher quality) or return nullptr to indicate
    // that there is no work to do. |frame| may be nullptr, which is equivalent
    // to a frame with an empty updated_region(). |done| callback may be called
    ...
    virtual void Encode(std::unique_ptr<webrtc::DesktopFrame> frame,
                        const FrameParams &param, EncodeCallback done) = 0;
};
